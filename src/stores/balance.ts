  ...
  persist<BalanceState>(
    (set, get) => ({
      ...
      calculateDailyBalance: () => {
        ...
        const daysUntilNextPayment = differenceInDays(
          nextPaymentDate,
          paymentDate
        );
        ...
        const pageDates = allDates.slice(
          startIndex,
        ...
        for (let i = 0; i < pageDates.length; i++) {
          ...
          const totalAvailable =
            baseDailyBalance + previousBalance + totalIncomesAmount;
          ...
          displayedDays.push({
            ...
            previousDayLeftover:
              i === 0
                ? accumulatedBalance
                : displayedDays[i - 1].remainingBalance,
      ...
      calculateFutureProjections: () => {
        ...
        const nextPaymentDate = addMonths(paymentDate, 1);
        const baseDailyBalance =
          initialBalance / differenceInDays(nextPaymentDate, paymentDate);
        ...
        while (currentDate < nextPaymentDate) {
          ...
          const projectedExpenses = dayExpenses.reduce(
            (acc, expense) => acc + expense.amount,
            0
          );
          const projectedIncomes = dayIncomes.reduce(
            (acc, income) => acc + income.amount,
            0
          );
          const totalAvailable =
            baseDailyBalance + previousBalance + projectedIncomes;
        ...
        const totalProjectionPages = Math.ceil(
          futureProjections.length / PROJECTIONS_PER_PAGE
        );
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          // Convertendo as strings ISO de volta para objetos Date no timezone local
          const userTimezone = getUserTimezone();
          state.expenses = state.expenses.map((expense) => ({
            ...expense,
            date: fromUTC(expense.date as unknown as string, userTimezone),
          }));
          state.incomes =
            state.incomes?.map((income) => ({
              ...income,
              date: fromUTC(income.date as unknown as string, userTimezone),
            })) || [];
          if (state.paymentDate) {
            state.paymentDate = fromUTC(
              state.paymentDate as unknown as string,
              userTimezone
            );
          }
